...
import threading
# run to create the engine.exe: pyinstaller -F engine.py --hidden-import pywin32 --hidden-import notify-py --uac-admin
app = Flask(__name__)
...
ByteAvengerScanner = Scanner(signatures=signaturesData, rootPath=app.root_path)


def startSystemWatcher(thread_resume):
    thread_resume.set()
    systemWatcher(ByteAvengerScanner,SYSTEM_DRIVE,thread_resume)
...
def scans():
    ...
    if SCAN_TYPE=="Quick":
        ...
        print("SanLoations", SCAN_LOCATIONS)
    ...
    scanReport = ByteAvengerScanner.scanFolders(location=SCAN_LOCATIONS)
    return scanReport
...
def quarFile():
    ...
    ByteAvengerScanner.quar.quarantine(originalPath, detectionSpace)
    return "Done!"
...
def restoreFile():
    ...
    thread_resume.clear()
    ByteAvengerScanner.quar.restore(originalPath)
...
def removeFile():
    ...
    ByteAvengerScanner.quar.remove(originalPath)
    return "Done"
...
def addFirewallRules(url):
    ...
    try:
        ...
        ips = response.split("\n")
        rule = "netsh advfirewall firewall delete rule name='ByteAvenger_AV_IP_RULE'"
        ...
        for ip in ips:
            if ip and ip[0] != '!' and "#" not in ip:
                try:
                    ...
                    rule = "netsh advfirewall firewall add rule name='ByteAvenger_AV_IP_RULE' Dir=Out Action=Block RemoteIP="+ip.rstrip()
                    # print(rule)
        ...
        try:
            for content in os.listdir(target):
                ...
                if age/3600 >= 24:
                    ...
                    yield f'data: {"Successfully Removed Files from: "+ os.path.join(target,content) + " Size: "+str(size)} \n\n'
